		...
		if (line.find("matched") != -1 and line.find("matched_p4") == -1) or (line.find("truth") != -1 and line.find("truth_p4") != -1):
			...
			if line[3:17] == "TLorentzVector":
				...
				for lorentzline in TLorentzVector_Leaves:
					...
					Histo_Book_Functions_AutoGen.write("\t\th_" + leafName + "_" + TLorentzName + "_EXCEPT_FINE = new TH1F(\"h_" + leafName + "_" + TLorentzName + "_EXCEPT_FINE\",\"\", 1000, min, max);\n")
			...
			#If the leaf is anything else
				...
				Histo_Book_Functions_AutoGen.write("\th_" + leafName + "_EXCEPT_FINE = new TH1F(\"h_" + leafName + "_EXCEPT_FINE\", \"\", 1000, min, max);\n")
...
for line in Custom_Histos:
	...
	if line[0:2] == "##": # If the line is a comment for separating things out
		...
		Histo_Custom_Book_Functions.write("\th_" + line[0:colonPos] + "_EXCEPT_FINE = new TH1F(\"h_" + line[0:colonPos] + "_EXCEPT_FINE\", \"\", 1000, min, max);\n")
