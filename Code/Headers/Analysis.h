...
void MC_Analysis::BookHistos() {
	...
	int Test_Polar_Plot_xMin = -1;
	int Test_Polar_Plot_xMax = 2;
	int Test_Polar_Plot_yMin = 0;
	int Test_Polar_Plot_yMax = 200;

	int Mass_Favour_Combination_2D_xMin = 0 - pi;
	int Mass_Favour_Combination_2D_xMax = 0 + pi;
	int Mass_Favour_Combination_2D_yMin = 0;
	int Mass_Favour_Combination_2D_yMax = 100;
	...
	*/
	...
	if (AnalysisType == "ElectronMuon" || AnalysisType == "ElectronTau" || AnalysisType == "MuonTau"){
		...
		//Work out if the common cuts are true here
	 	if (Z_mass_condition && combined_lepton_pt && leading_jets_invariant_mass && ptvarcone_40_0 && ptvarcone_40_1 && phi_int_condition) {
			common_cuts = true;
		}
